...
- 设置页与 README 更新 Translate for Zotero 服务建议：推荐使用 google，其次使用必应。
- 将插件与依赖锁文件版本统一更新为 0.5.0。

## v0.3.0

- 修复插件安装或启用后，已打开和延迟加载的 PDF 阅读器不显示工具栏按钮、需要重启 Zotero 才能使用的问题。
- 修复阅读模式插入或更新译文后，原文高亮仍停留在旧坐标、与文字错位的问题；字体、图片、窗口尺寸和译文高度变化后会同步刷新标注位置。
- 移除 Bilingual Reader 的 Translate for Zotero 具体服务选项，固定跟随 Translate for Zotero 当前默认服务；旧版保存的具体服务配置不再覆盖默认服务。
- 设置页提示推荐在 Translate for Zotero 中使用必应翻译服务。
- 增加工具栏恢复与 Zotero 原生阅读模式高亮对齐回归测试。
